...
# Introduction
To understand how population-level trends over time in smoking prevalence arise from trends in smoking at the individual-level requires reliable estimates of the probabilities of smoking initiation and quit by age and period. Due to the strong patterning of smoking by sex and socio-economic conditions, investigation of trends in smoking initiation and quitting should also be stratified by these factors. The problem is how to estimate the long-term trends in these smoking transition probabilities when we do not have a long-term dataset that tracks individuals over their lifecourses. The solution is to infer smoking transition probabilities from multiple years of cross-sectional survey data, which commonly provide data on the proportion of individuals who are current, former or never smokers at each age, and ask respondents to recall when they initiated and quit smoking. To estimate smoking transition probabilities reliably from these data requires correcting for biases by applying a range of methods and using external data sources to provide information not available in the cross-sectional surveys.    

> See the accompanying research article for full methods and methodology.  

In the `smktrans` R package, we developed a set of functions to use in producing estimates of the probabilities of smoking initiation and quit in England using data from the HSE 2001-2016. The code below illustrates the use of those functions.    
...
quit_data <- quit_est(
  ...
  relapse_dataq = quitcalc_relapse_data,
